...
@router.post("/entry-extract-plate",response_model=Message)
async def entry_extract_plate(file: UploadFile = File(...),)-> Message:
    try:
...
@router.post("/exit-extract-plate",response_model=Message)
async def exit_extract_plate(file: UploadFile = File(...),)-> Message:
    try:
        temp_path = f"temp_{file.filename}"
        with open(temp_path, "wb") as buffer:
            shutil.copyfileobj(file.file, buffer)
